    ...
    public class TiledGBDKMap : IComparable<TiledGBDKMap>
    {
            ...
            foreach (TiledObject tiledObject in gbdkExportEnabledTiledObjects)
            {
                ...
                foreach (TiledProperty prop in tiledObject.properties)
                {
                    ...
                    else if (prop.name.StartsWith(TILED_GBDK_PROPERTY_PREFIX))
                    {
                        ...
                        {
                            Console.WriteLine("Extra property specified on " + tiledObject.id + "(" + tiledObject.name + "): " + gbdkProp + ". All properties must be passed in via --object-property parameters, along with a default name and default value.");
                        }
                        else
                        {
        ...
        private void WriteStructHFile()
        {
            ...
            if (Configuration.generateObjectStruct)
            {
                ...
                File.WriteAllLines(Configuration.FullHeaderOutPath + "/" + Configuration.gbdkObjectStructName + ".h", hFileLines);
            }
            ...
            if (Configuration.generateMapStruct)
            {
                ...
                hFileLines.Add("#endif");
                File.WriteAllLines(Configuration.FullHeaderOutPath + "/" + Configuration.gbdkMapStructName + ".h", hFileLines);
                System.Console.WriteLine(Configuration.FullHeaderOutPath + "/" + Configuration.gbdkMapStructName + ".h");
            }
        ...
        private void WriteHFile()
        {
            ...
            File.WriteAllLines(Configuration.FullHeaderOutPath + "/" + mapIdentifier + ".h", hFileLines);
            System.Console.WriteLine(Configuration.FullHeaderOutPath + "/" + mapIdentifier + ".h");
        }
...
;
            ...
            File.WriteAllLines(Configuration.FullSourceOutPath+"/"+mapIdentifier + ".c", cFileLines);
            System.Console.WriteLine(Configuration.FullSourceOutPath + "/" + mapIdentifier + ".c");
        }
        ...
        public void RasterizeTMXFile()
        {
            ...
            Utils.RunProcess(Configuration.tiledInstallationPath + "/tmxrasterizer.exe", String.Join(" ", hiddenLayers) + " \"" + tmxFile + "\" " + Configuration.FullSourceOutPath + "/" + mapIdentifier + "_tilemap.png");
            Utils.RunProcess(Configuration.gbdkInstallationPath + "/bin/png2asset.exe", Configuration.FullSourceOutPath + "/" + mapIdentifier + "_tilemap.png -c \"" + Configuration.FullSourceOutPath + "/" + mapIdentifier + "_tilemap.c\" -map -use_map_attributes ");

            // png2asset.exe puts source and headers in the same folder
            // Move the .header file to where we want
            if (File.Exists(Configuration.FullSourceOutPath + "/" + mapIdentifier + "_tilemap.h")) {
                if(File.Exists(Configuration.FullHeaderOutPath + "/" + mapIdentifier + "_tilemap.h")){
                    File.Delete(Configuration.FullHeaderOutPath + "/" + mapIdentifier + "_tilemap.h");
                }
                File.Move(Configuration.FullSourceOutPath + "/" + mapIdentifier + "_tilemap.h", Configuration.FullHeaderOutPath + "/" + mapIdentifier + "_tilemap.h");
            }
